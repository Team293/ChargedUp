...
public final class SPIKE293Utils {
    ...
    public static double feetPerSecToControllerVelocity(double feetPerSec) {
        return ((feetPerSec * DrivetrainConstants.GEARBOX_RATIO_TO_ONE
                * DrivetrainConstants.ENCODER_UNITS_PER_REVOLUTION) / (DrivetrainConstants.WHEEL_CIRCUMFERENCE_FEET))
    ...
    public static double controllerUnitsToFeet(double encoderUnits) {
        return (encoderUnits * DrivetrainConstants.WHEEL_CIRCUMFERENCE_FEET)
                / (DrivetrainConstants.GEARBOX_RATIO_TO_ONE * DrivetrainConstants.ENCODER_UNITS_PER_REVOLUTION);
    }
    ...
    public static double feetToControllerUnits(double feet) {
        ...
        controllerUnits = ((feet * DrivetrainConstants.GEARBOX_RATIO_TO_ONE
                * DrivetrainConstants.ENCODER_UNITS_PER_REVOLUTION) / (DrivetrainConstants.WHEEL_CIRCUMFERENCE_FEET));
    ...
    public static double convertRPMToControllerVelocity(double rpm) {
        return (rpm * LauncherConstants.GEAR_RATIO * LauncherConstants.ENCODER_UNITS_PER_REVOLUTION)
                / LauncherConstants.MINUTES_TO_DECISECONDS;
    }
    ...
    public static double convertControllerVelocityToRPM(double velocity) {
        return (velocity * LauncherConstants.MINUTES_TO_DECISECONDS)
                / (LauncherConstants.ENCODER_UNITS_PER_REVOLUTION * LauncherConstants.GEAR_RATIO);
    }
}
