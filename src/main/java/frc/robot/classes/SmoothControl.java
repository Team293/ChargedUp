...
/**
 * This is an abstracted version of the Smooth Control algorithm. The class
 * contains methods to calculate the required turn rate for the robot to follow
 * a path. It is only used by certain other commands and the documentation is
 * listed within this class.
 */
public class SmoothControl {
    ...
    public double computeTurnRate(Position2D currentPose, Position2D targetPose, double givenMaxVelocity,
            boolean inReverse) {
        ...
        if (inReverse) {
            // Add PI to the current and target headings to "trick" smooth control into
            // thinking we're facing forwards
            poseHeading += Math.PI;
